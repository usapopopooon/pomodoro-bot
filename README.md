...
- [tests/ui/test_panel_views.py](tests/ui/test_panel_views.py) — Control Panel(9 ボタン) / Phase Panel(3 ボタン) / モーダル / NotificationSettingsView のラベル同期 / 使い方 embed / オーナーガード / 入力バリデーション
- [tests/test_config.py](tests/test_config.py) — URL 正規化 / CSV パース / token 必須
- [tests/test_room_manager.py](tests/test_room_manager.py) — setup → running 遷移、begin_phases ガード、自然タイムアウトの phase-end path、マルチルーム独立、並行デッドロック回避、オーナー委譲、参加者別 🍅 記録、per-phase 通知トグルの owner-only、`IntegrityError` 経路
