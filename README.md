...
**2. Phase Panel(毎フェーズ投稿)** — オーナーが ▶️ 開始 を押すと、以降フェーズ切替のたびにチャンネルに**新しいメッセージ**が届く。円形タイマー画像 + 3 ボタン。

```
🍅 作業 フェーズ開始!(25 分間集中 → 休憩 5 分)
...
Python 3.12 / discord.py ~2.6 / PostgreSQL 17 / SQLAlchemy 2.x (async) + asyncpg / Alembic / pydantic-settings / **Pillow**(タイマー画像生成)/ Docker Compose / Railway
...
> `Attach Files` は Phase Panel の円形タイマー PNG を送るのに必要です。
...
| Phase Panel の画像が送れない | Bot に **Send Messages** / **Embed Links** / **Attach Files** 権限があるか確認 |
...
└── ui/                  # Discord UI
    ├── embeds.py        #   control_panel_embed / ended_embed / phase_announcement_content
    ├── panel_views.py   #   ControlPanelView + PhasePanelView + OptionsView + modals
    └── timer_image.py   #   円形タイマー PNG 生成(Pillow + DejaVu Sans)
...
- [tests/ui/test_embeds.py](tests/ui/test_embeds.py) — Control Panel embed / phase 切替メッセージ / 参加者表示
- [tests/ui/test_panel_views.py](tests/ui/test_panel_views.py) — Control Panel(7 ボタン) / Phase Panel(3 ボタン) / モーダル / オーナーガード / 入力バリデーション / 円形 PNG 生成
...
| やりたいこと | どこを触るか |
|---|---|
| 統計グラフ画像 | `pomodoros` に `room_id` / `user_id` / `duration_seconds` / `completed_at` が揃っている。`services/room_service.py` にクエリを足し、Pillow / matplotlib で描画して Phase Panel の Options 経由で配信 |
