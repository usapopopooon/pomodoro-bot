...
[🙋 参加] [🚪 退出] [✍️ タスク] [📊 統計] [❓ 使い方]
[▶️ 開始]  [⚙️ 時間設定] [🔔 通知] [🔊 ボイス] [🛑 終了]   ← オーナー専用
```

**2. Phase Panel(毎フェーズ投稿)** — ▶️ 開始 後、フェーズ境界(自然終了 / スキップ / プラン更新)でチャンネルに**新しいメッセージ**が届く。ASCII プログレスバーが **`POMO_REFRESH_MINUTES` 分おきに edit で更新**(既定 1 分)されてバーが伸びていく。終了までの残り時間は Discord の `<t:…:R>` で**クライアント側が秒単位でライブカウントダウン**。🔔 通知が ON のフェーズでは先頭に**スポイラー付きメンション**(`||<@uid> …||`)が乗り、ping を発火しつつユーザ名は隠す。

```
||<@111> <@222>||        ← 🔔 通知 ON のフェーズだけ。クリックで展開、通知は発火済み
⏰ **作業**
`████░░░░░░░░░░░░░░░░ 5分 / 25分`
...
- **ボイスチャンネル合図** — `🔊 ボイス` でオーナーの VC に Bot を接続、フェーズ境界 / 残り 1 分 / pause-resume / 終了で `voices/*.wav` を再生。音声 OFF なら全パスを no-op で素通り
...
| 🔔 通知 | オーナー | フェーズ別(作業/短休憩/長休憩)にスポイラー付きメンションの ON/OFF を切替。既定は全 ON |
...
| `start.wav` | ▶️ 開始 直後(`start-task.wav` は重複なので付けない) |
| `alarm.wav` → `end-task.wav` → `start-break.wav` | WORK 終了 → 短休憩開始 |
| `alarm.wav` → `end-task.wav` → `start-long-break.wav` | WORK 終了 → 長休憩開始 |
| `end-break.wav` → `start-task.wav` | 短休憩 → WORK(alarm 無し) |
| `end-long-break.wav` → `start-task.wav` | 長休憩 → WORK(alarm 無し) |
| `start-X.wav` (単発) | ⏭ スキップ時(end- や alarm は無し — ユーザの能動操作) |
| `pause.wav` / `resume.wav` | ⏸ 一時停止 / 再開 |
| `one-minute-left.wav` | フェーズ終了 60 秒前(各フェーズ 1 回) |
| `end.wav` | 🛑 終了(オーナー操作) |
| `auto-end.wav` | 全員退出による自動終了 |

`alarm.wav` は **WORK 終了時のみ** 鳴らします。フォーカスを切る瞬間の注意喚起としては効くが、休憩明けは静かに「お仕事に戻りましょう」と言うほうが自然なので。

`bot_restart` / `superseded` / `shutdown` / `error` での終了は無音(聞き手がいない / 急ぎたい)。

## 技術スタック

Python 3.12 / discord.py[voice] ~2.6 (PyNaCl 同梱) / ffmpeg + libopus + libsodium / PostgreSQL 17 / SQLAlchemy 2.x (async) + asyncpg / Alembic / pydantic-settings / Docker Compose / Railway
...
voices/                  # 14 本の .wav 音声合図(start / end / alarm / pause / resume / one-minute-left / connected / …)
...
| `room_events` | ライフサイクルの append-only outbox | `phase_completed` / `ownership_transferred` / `timer_started` / `notify_updated` 等を tail 可 |
...
- [tests/test_voice_manager.py](tests/test_voice_manager.py) — VC 接続 / 移動 / 切断 / 一括切断 / 再生中の差し替え / 未接続時の no-op
- [tests/test_room_manager.py](tests/test_room_manager.py) — setup → running 遷移、begin_phases ガード、自然タイムアウトの phase-end path、マルチルーム独立、並行デッドロック回避、オーナー委譲、参加者別 🍅 記録、per-phase 通知トグル、🔊 ボイストグル(owner / 非 VC / 接続失敗)、フェーズ境界の音声合図(end-X→alarm→start-Y / 長休憩 / pause-resume / skip / one-minute-left / 自動終了)、`bot_user_id` 永続化、`IntegrityError` 経路
