...
| `start.wav` + `start-task.wav` | ▶️ 開始 直後(タイマー開始 → 最初の WORK) |
| `end-task.wav` → `alarm.wav` → `start-break.wav` | WORK 終了 → 短休憩開始 |
| `end-task.wav` → `alarm.wav` → `start-long-break.wav` | WORK 終了 → 長休憩開始 |
| `end-break.wav` → `alarm.wav` → `start-task.wav` | 短休憩 → WORK |
| `end-long-break.wav` → `alarm.wav` → `start-task.wav` | 長休憩 → WORK |
